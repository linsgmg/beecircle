...
import com.lin.baselib.widgt.klinechart.base.IAdapter;
...
import com.lin.baselib.widgt.klinechart.draw.MainDraw;
import com.lin.baselib.widgt.klinechart.draw.Status;
import com.lin.baselib.widgt.klinechart.entity.IKLine;
import com.lin.baselib.widgt.klinechart.formatter.TimeFormatter;
import com.lin.baselib.widgt.klinechart.formatter.ValueFormatter;
...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    private Paint mGridPaint2 = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    private void init() {
        ...
        mGridPaint2.setColor(Color.parseColor("#1A1A2A"));
        mGridPaint2.setStrokeWidth(DensityUtil.dip2px( getContext(),10));
    ...
    private void drawGird(Canvas canvas) {
        ...
        for (int i = 0; i <= mGridRows; i++) {
            if (i < mGridRows) {
                ...
                canvas.drawLine(0, rowSpace * i + mMainRect.top, mWidth, rowSpace * i + mMainRect.top, mGridPaint2);
            }
        }
        //-----------------------下方子图------------------------
        if (mChildDraw != null) {
            canvas.drawLine(0, mVolRect.bottom, mWidth, mVolRect.bottom+10, mGridPaint2);
            canvas.drawLine(0, mChildRect.bottom, mWidth, mChildRect.bottom+10, mGridPaint2);
        } else {
            canvas.drawLine(0, mVolRect.bottom, mWidth, mVolRect.bottom+10, mGridPaint2);
        }
        //纵向的grid
    ...
    private void drawK(Canvas canvas) {
        //保存之前的平移，缩放
        canvas.save();
        canvas.translate(mTranslateX * mScaleX - 150, 0);
    ...
    private void drawText(Canvas canvas) {
        ...
        for (int i = 1; i < mGridColumns; i++) {
            float translateX = xToTranslateX(columnSpace * i) + 150;
        ...
        float translateX = xToTranslateX(0) + 150;
        if (translateX >= startX && translateX <= stopX) {
            canvas.drawText(getAdapter().getDate(mStartIndex), 0, y, mTextPaint);
        }
        translateX = xToTranslateX(mWidth) + 150;
        if (translateX >= startX && translateX <= stopX) {
    ...
    private void drawMaxAndMin(Canvas canvas) {
        if (!mainDraw.isLine()) {
            ...
            //绘制最大值和最小值
            float x = translateXtoX(getX(mMainMinIndex)) - 150;
            ...
            x = translateXtoX(getX(mMainMaxIndex)) - 150;
